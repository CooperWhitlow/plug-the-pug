...
function sys_shutdown() {
  ajax_status.open("GET", "http://plugthepug.ngrok.io/cmd_func.php?cmd=shutdown", true);
  ajax_status.send();
}

function sys_reboot() {
  ajax_status.open("GET", "http://plugthepug.ngrok.io/cmd_func.php?cmd=reboot", true);
  ajax_status.send();
}
...
function reload_img () {
  if(!halted) mjpeg_img.src = "http://plugthepug.ngrok.io/cam_pic.php?time=" + new Date().getTime() + "&pDelay=" + preview_delay;
  else setTimeout("reload_img()", 500);
}

function error_img () {
  setTimeout("mjpeg_img.src = 'http://plugthepug.ngrok.io/cam_pic.php?time=' + new Date().getTime();", 100);
}
...
function updatePreview(cycle)
{
   if (mjpegmode)
   {
      if (cycle !== undefined && cycle == true)
      {
         ...
         setTimeout("mjpeg_img.src = \"http://plugthepug.ngrok.io/cam_pic_new.php?time=\" + new Date().getTime()  + \"&pDelay=\" + preview_delay;", 1000);
         return;
      }
      ...
      if (previous_halted != halted)
      {
         if(!halted)
         {
            mjpeg_img.src = "http://plugthepug.ngrok.io/cam_pic_new.php?time=" + new Date().getTime() + "&pDelay=" + preview_delay;			
         }
         else
         {
            mjpeg_img.src = "http://plugthepug.ngrok.io/unavailable.jpg";
         }
      }
